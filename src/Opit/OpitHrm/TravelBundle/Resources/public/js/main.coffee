$.extend true, $(document).data('opithrm'),
    funcs:
        ...
        changeTravelExpenseStatus: (data, $spinner) ->
            $.ajax
                method: 'POST'
                url: Routing.generate 'OpitOpitHrmTravelBundle_expense_state'
                data: data
                global: false
            .done (data) ->
                location.reload()
        ...
        changeTravelRequestStatus: (data, $spinner) ->
            if $spinner is undefined
                ...
                $spinner = $(document).data('opithrm').funcs.disableStatusDropdown($dropDown)
            reloadPage = false
            $.ajax
                method: 'POST'
                url: Routing.generate 'OpitOpitHrmTravelBundle_request_state'
                data: data
            .done (data) ->
                if data is 'error'
