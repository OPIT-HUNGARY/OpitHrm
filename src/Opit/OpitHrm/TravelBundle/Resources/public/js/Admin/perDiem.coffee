...
$(document).ready () ->
    ...
    $.ajax
        method: 'GET'
        url: Routing.generate 'OpitOpitHrmTravelBundle_admin_show_perdiem'
    .done (data) ->
        perDiemWrapper = data
        return
    return
...
$('#save').click (event) ->
    ...
    if form.valid() && validationOfHoursCompare() && (isHasChild || isHadChild)
        ...
        $.ajax
            ...
            data: form.serialize()
            dataType: 'json'
            url: Routing.generate 'OpitOpitHrmTravelBundle_admin_save_perdiem'
        .done (data) ->
            disableSaveButton(false)
            response = data
            ...
            $.ajax
                type: 'POST'
                url: Routing.generate 'OpitOpitHrmTravelBundle_admin_list_perdiem'
                data: "showList" : 1
            .done (data)->
                $('.container').html data
                $(document).data('opithrm').funcs.showAlert $('#main-content'), response, "create", "Per diems saved successfully!"
                isHadChild = $('.container').children('.formFieldsetChild').length > 0
