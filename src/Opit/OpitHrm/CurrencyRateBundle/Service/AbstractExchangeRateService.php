...
namespace Opit\OpitHrm\CurrencyRateBundle\Service;
...
abstract class AbstractExchangeRateService implements ExchangeRateInterface
{
    ...
    public function getExchangeRates($options = array())
    {
        ...
        return $this->em->getRepository('OpitOpitHrmCurrencyRateBundle:Rate')->findAllByDates(
            $options['startDate'],
            $options['endDate']
        );
    }
    ...
    public function getRatesByDate(\DateTime $date = null)
    {
        ...
        return $this->em->getRepository('OpitOpitHrmCurrencyRateBundle:Rate')->getRatesArray($date);
    }
    ...
    {
        $rate =  $this->em->getRepository('OpitOpitHrmCurrencyRateBundle:Rate')->findLastRate();
    ...
    {
        $rate =  $this->em->getRepository('OpitOpitHrmCurrencyRateBundle:Rate')->findFirstRate();
    ...
    public function getMissingExchangeRates($options)
    {
        ...
        $options = array(
            ...
            'currencyCodes' => implode(
                ',',
                $this->em->getRepository('OpitOpitHrmCurrencyRateBundle:Currency')->getAllCurrencyCodes()
            )
        );
     ...
     *
     * @return \Opit\OpitHrm\CurrencyRateBundle\Entity\Rate|null $rate object or null.
     */
    protected function createOrUpdateRate($currencyCode, $value, \DateTime $dateObj, $force)
    {
        if (!$this->em->getRepository('OpitOpitHrmCurrencyRateBundle:Rate')->hasRate($currencyCode, $dateObj)) {
            $rate = new Rate();
            $currency = $this->em->getRepository('OpitOpitHrmCurrencyRateBundle:Currency')
