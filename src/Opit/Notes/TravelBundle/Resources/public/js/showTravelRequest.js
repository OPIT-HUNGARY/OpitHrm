...
(function() {
  var $accomodationCollection, $accomodationParent, $addAccomodation, $addDestination, $cost, $currency, $destinationCollection, $destinationParent, $div, $errorMessages, $form, accomodationsLabel, addForm, addFormDeleteButton, compareDays, destinationsLabel, generalData, numberOfNightsListener, requiredApprovals, travelCustomer, travelRequestAccomodations0, travelRequestDestinations0;
  ...
  compareDays = function() {
    ...
    if (diffDays > 0 && accomodationDays > diffDays) {
      if ($accomodationWrapper.children('label.error-label').length === 0) {
  ...
  generalData.append($('#travelRequest_user_ac, #travelRequest_departure_date, #travelRequest_arrival_date, #travelRequest_customer_related, #travelRequest_customer_name, #travelRequest_trip_purpose').parent());
  ...
  if ($('#travelRequest_destinations :input[type=text]').length > 1) {
    $('#travelRequest_destinations').children().each(function() {
      ...
      $(this).addClass('padding-10 margin-left-1-em margin-bottom-1-em display-inline-block vertical-align-top');
      $(this).children().remove('label');
      $(this).append(addFormDeleteButton);
      $currency = $(this).find('.currency');
      $cost = $(this).find('.cost');
      return $cost.parent().append($currency);
    });
  } else {
