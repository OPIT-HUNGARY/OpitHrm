...
namespace Opit\Notes\LeaveBundle\Security\Authorization\Voter;
...
class LeaveAccessVoter implements VoterInterface
{
    ...
    {
        $supportedClass = 'Opit\Notes\LeaveBundle\Entity\LeaveRequest';
    ...
    public function vote(TokenInterface $token, $leaveRequest, array $attributes)
    {
        ...
        $leaveRequestStatus = $this->entityManager->getRepository('OpitNotesLeaveBundle:StatesLeaveRequests')->getCurrentStatus($leaveRequest->getId());
    ...
    {
        return $this->entityManager->getRepository('OpitNotesUserBundle:Groups')->findUserGroupsArray($user->getId());
    }
}
